...
public class AutoScrollViewPager extends ViewPager {
    ...
    private float mConflictTouchX = 0f, mConflictDownX = 0f, mConflictTouchY = 0f, mConflictDownY = 0f;
    private CustomDurationScroller scroller                    = null;

    public static final int        SCROLL_WHAT                 = 0;

    //是否处理滑动冲突
    private boolean mIsHandleSlipConflict = false;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (mIsHandleSlipConflict) {
            switch (action) {
                case MotionEvent.ACTION_DOWN:
                    mConflictDownX = ev.getX();
                    mConflictDownY = ev.getY();
                    break;
                case MotionEvent.ACTION_MOVE:
                    mConflictTouchX = ev.getX();
                    mConflictTouchY = ev.getY();
                    if (Math.abs(mConflictDownX - mConflictTouchX) < Math.abs(
                        mConflictDownY - mConflictTouchY)) {
