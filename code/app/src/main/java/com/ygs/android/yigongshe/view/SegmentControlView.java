...
//source https://github.com/Carbs0126/AndroidSegmentControlView/blob/master/SegmentControlView
...
public class SegmentControlView extends View {
  ...
  private void initAttr(Context context, AttributeSet attrs) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (attr == R.styleable.SegmentControlView_scv_BackgroundSelectedColor) {
        ...
        mSegmentPaddingHorizontal =
            a.getDimensionPixelSize(attr, DEFAULT_SEGMENT_PADDING_HORIZONTAL);
  ...
  private void init() {
    ...
    if (mTextSize == 0) mTextSize = sp2px(getContext(), DEFAULT_TEXT_SIZE_SP);
  ...
  public void setTexts(String[] texts) {
    ...
    if (texts == null || texts.length < 2) {
      throw new IllegalArgumentException(
          "SegmentControlView's content text array'length should larger than 1");
    }
  ...
  private void drawBackgroundAndFrameAndText(Canvas canvas) {
    ...
    for (int i = 0; i < mTexts.length; i++) {
      ...
      if (i == 0) {
        ...
        rectFArc.offsetTo(rectF.right - 2 * mFrameCornerRadius,
            rectF.bottom - 2 * mFrameCornerRadius);
      ...
      if (mIsGradient) {
        if (viewPagerPositionOffset != 0f) {
          if (i == viewPagerPosition) {
            curBackgroundColor = getEvaluateColor(viewPagerPositionOffset, mColorBackgroundSelected,
                mColorBackgroundNormal);
            curTextColor =
                getEvaluateColor(viewPagerPositionOffset, mColorTextSelected, mColorTextNormal);
          } else if (i == viewPagerPosition + 1) {
            curBackgroundColor = getEvaluateColor(viewPagerPositionOffset, mColorBackgroundNormal,
                mColorBackgroundSelected);
            curTextColor =
                getEvaluateColor(viewPagerPositionOffset, mColorTextNormal, mColorTextSelected);
          } else {
            curBackgroundColor = mColorBackgroundNormal;
            curTextColor = mColorTextNormal;
          }
      ...
      if (curTouchedIndex == i) {
        paintBackground.setColor(
            getDarkColor(curBackgroundColor, TOUCHED_BACKGROUND_DARK_COEFFICIENT));
      }
      canvas.drawPath(pathFrame, paintBackground);
      canvas.drawPath(pathFrame, paintFrame);

      paintText.setColor(curTextColor);
      canvas.drawText(mTexts[i], left + unitWidth / 2, rectF.centerY() + textCenterYOffset,
          paintText);
    }
  }
