...
public class MineRegistActivity extends BaseActivity {
    ...
    public void regist() {
        ...
        if (CheckLegalUtils.checkPhone(phone)
                && CheckLegalUtils.checkCode(code)
                && CheckLegalUtils.checkPwd(pwd)
                && CheckLegalUtils.checkName(name)) {
            PersonalAgent.postRegistByPhone(phone, code, pwd, name, new JsonResponseHandler<LoginResult.UserInfoContent>() {
                @Override
                public void success(LoginResult.UserInfoContent data) {
                    Toast.makeText(MineRegistActivity.this, "注册成功", Toast.LENGTH_SHORT).show();
                    back();
                }
            });
